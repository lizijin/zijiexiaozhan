 ...
 */
class NotifyNoneStableIdRecyclerViewActivity : AppCompatActivity() {
...
//        mRecyclerView.setItemViewCacheSize(4)
    ...
    inner class MyAdapter(val mStrings: MutableList<String>) :
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
            println("RecyclerView 测试Notify Item时动画 onCreateViewHolder ")
        ...
        override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
            println("RecyclerView 测试Notify Item时动画 onBindViewHolder $position ${mStrings[position]}")
        ...
        override fun onViewRecycled(holder: RecyclerView.ViewHolder) {
            println("RecyclerView 测试Notify Item时动画 发生回收 " + (holder.itemView as TextView).text)
    ...
    fun notifyData(view: View) {
        if(findViewById<CheckBox>(R.id.checkbox).isChecked){
            (mRecyclerView.adapter as MyAdapter).mStrings.reverse();
        }
